...
function generateOptions(number) {
    ...
    let number2 = getRandomInt(Math.abs(number - number*50*(1/100)), number+number*50*(1/100));

    if (number2 === number) {
        number2 +=  getRandomInt(11, 21);
    }

    let number3 = getRandomInt(Math.abs(number - number*30*(1/100)), number+number*40*(1/100));
...
function generateSubtrahendBorrow(minuendNumber, digitInMinuend, digitInSubtrahend) {
    ...
    for (let i = digitInMinuend - digitInSubtrahend; i < digitInMinuend; ++i) {
        ...
        if (minuendNumberArray[i-1] === lastElementInNumber 
            && minuendNumberArray[i] === numberInArray[Math.abs(digitInMinuend - digitInSubtrahend - i)]) {
                numberInArray[Math.abs(digitInMinuend - digitInSubtrahend - i)] = getRandomInt(0, minuendNumberArray[i]);
        }
    }
...
function generateQuestions(noOfQ, digitInMinuend, digitInSubtrahend, isBorrow) {
    ...
    if (digitInMinuend === 1 && digitInSubtrahend === 1 && isBorrow) {
        return 'not Possible';
    }
...
module.exports = {
    ...
    generateOptions,
    generateSubtrahend,
    generateSubtrahendBorrow,
    generateSubtrahendNonBorrow,
}
